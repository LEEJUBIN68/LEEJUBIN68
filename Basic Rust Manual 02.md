...
```c++
// 1. 우리는 이전에 If문을 살펴보았다. 이제 반복문을 살펴보도록 하자.
// 2. loop라는 문법은 웬만해서는 다 알고있을 것이다.
// 3. 이 Rust 프로그램에도 마찬가지로, loop로 문자열을 돌리면 계속 반복되는 것이고,
      break를 걸면 한 줄만 출력되고 loop문에서 빠져나온다.
...
```c++
// 1. 'number'라는 변수에 3을 지정해두고, 'mut'를 기입하여, 숫자가 변할 수 있게 만들었다.
// 2. 'while' 조건문으로 'number'라는 변수가 0이 되기 전까지 계속해서 출력하라고
      Coding 하였다.
// 3. 출력되는 것은 'number' 변수일 것이고, 내림차순으로 "3, 2, 1"이 출력될 것이다. 그 이유는
      number 변수에서 -1씩 하도록 되어있기 때문이다.
// 4. 최종적으로, while문에서 반복이 끝나고 빠져나오면, 마지막 줄에는 "LiftOff"라는 문구가
      출력될 것이다.
...
```c++
// 1. 등차수열의 합(feat. 가우스):
// 2. 1 ~ 100의 합: 1 + 100 => 101, 2 + 99 => 101, 3 + 98 => 101, ...
// 3. 즉, 50번을 반복(중복 제외)해야지 1 ~ 100까지 더한 수가 나오므로, '50 x 101 = 5050'이라는 결과값이 나온다.
// 4. 이를 응용해서 만들어보자.
...
fn main() {
   ...
   while number <= 100 { // 100이하 일 때까지 계속 반복
      sum += number;
      // sum = sum + number;
      number += 1;
      // number = number + 1;
   ...
   println!("{}!\n", sum);
   // println!("5050"): 굳이 다 계산해서 이렇게 할 필요가 없다!
}
```
...
fn main() {
    ...
    while numPlus <= 99 {
        numPlus += 1;
        numMinus -= 1;
        initSum = numPlus + numMinus;
        finalSum += numPlus;
        
        println!("{} + {} = {}\n", numPlus, numMinus, initSum);
    }
...
```c++
// 1. a라는 배열에 Element가 5개가 들어가있다.
// 2. 그 중에서 0번째 Index부터 4번째 Index까지 출력하고 싶다.
// 3. 따라서, 우리는 Index 5미만까지 출력하도록 Code를 짰고,
      Index 번호를 1씩 추가해서 순서대로 출력되게끔 While문을 완성시켰다.
...
```c++
// 1. 결과값은 위와 동일하지만, 다르게 Coding 해보자.
// 2. 새로운 'iter'라는 것이 생겼다.
// 3. 이는 a라는 배열에 값을 차례대로 꺼내고, 반복
      가능하게 만들어준다는 의미이다.
...
// 1. 역순으로 출력하고 싶다면 어떨까?
// 2. 'rev'를 사용하면 된다.
// 3. 아래의 Code는 number 안에 1~4까지의 수를 넣고,
      역순으로 이 숫자들을 출력하겠다는 의미이다.
